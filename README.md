...
│   └── publish-index-mirror.yml    # 固定签名目录 Release 发布原语
...
│   └── metadata-v1.bundle.json # Release 入口使用的单文件签名信封
└── scripts/
...
6. 成功后由 `publish-index-mirror` 把信封原子替换到固定的 `registry-index-v1` Release。
...
Codex Themer 客户端只从固定的 `registry-index-v1` Release 拉取 `metadata-v1.bundle.json`，再用内置公钥验证信封中的元数据原文。Release 资产是可覆盖的传输入口，不是信任根；真实性只来自 Ed25519 签名。网络失败时客户端只回退最近一次验签通过的本机目录，不切换到第二套远程协议。客户端启动后只默认同步签名目录；主题包和预览图只在用户浏览或明确安装时下载，安装后不会自动应用。
