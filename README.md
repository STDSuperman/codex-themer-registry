...
│   └── metadata-v1.bundle.json # raw 入口使用的单文件签名信封
└── scripts/
...
Codex Themer 客户端只从本仓库 `main` 分支的 `raw.githubusercontent.com` 固定地址读取 `metadata/metadata-v1.bundle.json`，再用内置公钥验证信封中的元数据原文。raw 单文件只是传输容器，不是信任根；真实性只来自 Ed25519 签名。网络失败时客户端只回退最近一次验签通过的本机目录，不切换到第二套远程协议。客户端启动后只默认同步签名目录；主题包和预览图仍由 GitHub Release assets 承载，只在用户浏览或明确安装时下载，安装后不会自动应用。
